...
**① 安装时输入密码 —— 默认打码，按 F2 可看明文，全角字符实时报警**
...
2026-09-18 12:27:39 [INFO] 认证返回：Portal协议认证成功！（有线 10.145.145.232）
2026-09-18 12:27:39 [INFO] 门户已放行，正在等待网络真正连通（无线一般需要几秒）...
2026-09-18 12:27:41 [INFO] 校园网已连通，认证完成。
```

**③ 校园 Wi-Fi（BISTU）自动识别并认证 —— 网线拔掉也照常工作**

```
2026-09-17 13:06:22 [INFO] 尝试 WLAN 无线（SSID BISTU）：10.153.92.129
2026-09-17 13:06:22 [INFO] 认证返回：Portal协议认证成功！（无线 10.153.92.129）
2026-09-17 13:06:22 [INFO] 门户已放行，正在等待网络真正连通（无线一般需要几秒）...
2026-09-17 13:06:25 [INFO] 校园网已连通，认证完成。
```
...
```
=== 网络代理一键修复 ===
修复前：使用代理   代理地址：127.0.0.1:7897
已清理注册表代理残留（ProxyEnable / AutoDetect / ProxyServer）
修复后：直连   代理地址：(无)
✅ 代理已恢复正常（直连）。现在请完全关闭浏览器再重新打开。
```
